...
export default function CheckSymptoms() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      if (Object.keys(selectedOption).length > 2) {
  ...
  return (
    <>
      ...
      >
        {!session && (
          <label className="space-y-2 font-bold text-2xl flex flex-col items-left text-accent max-w-[600px] min-w-[300px] w-fit">
            <span>Name</span>
            <input
              id="name"
              name="name"
